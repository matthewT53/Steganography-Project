...
void BMPFile::hide(const std::string &input_filename, const std::string &password) const
{
    ...
    if (able_to_store(get_file_size(), input_size)){
        ...
        store_in_image((byte *) StegFile::buffer_ + header_size_, (byte *) size_buffer, 4);

        // store the input file
        // an int is 32 bits and each byte takes 2 of these bits so 16 bytes are required.
        store_in_image((byte *) (StegFile::buffer_ + header_size_ + 16), (byte *) bin.get_buffer(), bin.get_file_size());
...
void BMPFile::reveal(const std::string &output_filename, const std::string &password) const
{
    ...
    extract_bits((byte *)StegFile::buffer_ + header_size_, size_buffer, 4);
    ...
    extract_bits((byte *) (StegFile::buffer_ + header_size_ + 16), (byte *)output_buffer, output_size);

    // write buffer to file
    write_to_file(output_buffer, output_size, output_filename + "_revealed");
