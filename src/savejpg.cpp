...
/***************************************************************************/
void processRgb(unsigned char *rgbBuffer, unsigned int bufSize, WORD w, WORD h); // written by MatthewT53
...
{
	char BMP_filename[64]; // MatthewT53: Potential buffer overflow here
	...
	printf("Ximage_ori: %u and Yimage_ori: %u\n", Ximage_original, Yimage_original);
...
// MatthewT53: Converts an rgb buffer into a jpeg image and then stores the image on disk
// basically a copy of the above function but doesn't need to load a bmp image
void RGBToJpeg(unsigned char *rgbBuffer, unsigned int rgbSize, const char *jpegFileName, unsigned int width, unsigned int height)
{
	WORD Ximage_original = width, Yimage_original = height;	//the original image dimensions,
											// before we made them divisible by 8

	bitstring fillbits; //filling bitstring for the bit alignment of the EOI marker

	processRgb(rgbBuffer, rgbSize, width, height);
	
	fp_jpeg_stream = fopen(jpegFileName,"wb");
	init_all();
	SOF0info.width=Ximage_original;
	SOF0info.height=Yimage_original;
	writeword(0xFFD8); //SOI
...
// MatthewT53: Based on the loadBmp function however skips opening the file.
void processRgb(unsigned char *rgbBuffer, unsigned int bufSize, WORD w, WORD h)
{
	WORD Xdiv8,Ydiv8;
	BYTE nr_fillingbytes;//The number of the filling bytes in the BMP file
	// (the dimension in bytes of a BMP line on the disk is divisible by 4)
	...
	for (nrline = 0; nrline < Yimage; nrline++)
	{
		// fread(RGB_buffer+nrline*Xdiv8, 1, Ximage*3, fp_bitmap);
		// fread(TMPBUF, 1, nr_fillingbytes, fp_bitmap);
		memcpy(RGB_buffer + nrline*Xdiv8, rgbBuffer + (nrline * 3*(Ximage)), Ximage*3);
		memcpy(TMPBUF, rgbBuffer, nr_fillingbytes);
		memcpy(&lastcolor,RGB_buffer+nrline*Xdiv8+Ximage-1,3);
		for (column = Ximage; column < Xdiv8; column++)
		{
			memcpy(RGB_buffer+nrline*Xdiv8+column,&lastcolor,3);
